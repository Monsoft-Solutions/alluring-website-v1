...
/**
 * The page's FAQ, e.g. "BBL questions, answered". The questions come from
 * `procedure.faqs`, the same list the page graph's `FAQPage` node and the
 * paid landing page read, so the visible answers and the structured data
 * cannot differ.
 *
 * Native `<details>`: every answer is in the HTML whether it is open or not,
 * and opens with no JavaScript. `module-kit.css` animates the height where
 * the browser can size to `auto`.
 */
export function ModuleFaq({
    ...
    return (
        <div className='pm-defer bg-white'>
            <section
                id='faq'
                aria-labelledby='faq-heading'
                className={cn(
                    moduleContainer,
                    'pm-faq scroll-mt-32 py-12 md:py-24 lg:grid lg:scroll-mt-40 lg:grid-cols-[22.5rem_minmax(0,47.5rem)] lg:justify-between lg:gap-x-20'
                )}
            >
                <h2
                    id='faq-heading'
                        ...
                        >
                            <summary className='flex min-h-11 cursor-pointer items-center justify-between gap-5 py-4.5'>
                                ...
                                <svg
                                    aria-hidden='true'
                                    viewBox='0 0 24 24'
                                    className='pm-plus size-5 shrink-0 text-stone-900'
                                    fill='none'
                                    stroke='currentColor'
                                    strokeWidth={1.6}
                                    strokeLinecap='round'
                                >
                                    <path d='M5 12h14' />
                                    <path className='pm-plus-v' d='M12 5v14' />
                                </svg>
                            </summary>
