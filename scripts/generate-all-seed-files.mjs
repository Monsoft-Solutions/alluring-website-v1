...
const imageResults = JSON.parse(
    fs.readFileSync('/tmp/all-image-processing-results.json', 'utf-8')
)
...
function inferCategoriesAndTags(slug, title, content) {
    ...
    // Categories
    if (
        lowerSlug.includes('bbl') ||
        lowerContent.includes('brazilian butt lift') ||
        lowerContent.includes('bbl')
    ) {
        categories.push('BBL')
    }
    if (
        lowerSlug.includes('breast') ||
        lowerContent.includes('breast augmentation') ||
        lowerContent.includes('breast reduction') ||
        lowerContent.includes('breast lift')
    ) {
    ...
    if (
        lowerSlug.includes('tummy') ||
        lowerContent.includes('tummy tuck') ||
        lowerContent.includes('abdominoplasty')
    ) {
        categories.push('Tummy Tuck')
    }
    if (
        lowerSlug.includes('mommy') ||
        lowerContent.includes('mommy makeover')
    ) {
        categories.push('Mommy Makeover')
    }
    if (
        lowerContent.includes('recovery') ||
        lowerContent.includes('post-op') ||
        lowerContent.includes('healing')
    ) {
    ...
    if (lowerSlug.includes('breast')) {
        if (lowerContent.includes('augmentation'))
    ...
    if (
        lowerContent.includes('swelling') ||
        lowerContent.includes('itching') ||
        lowerContent.includes('bruising')
    ) {
        tags.push('Swelling')
    }
    ...
    if (
        ...
        lowerContent.includes('lymphatic')
    ) {
        tags.push('Lymphatic Massage')
    }
    if (
        lowerTitle.includes('weeks') ||
    ...
    if (
        ...
        lowerContent.includes('sitting') ||
        lowerContent.includes('driving')
    ) {
...
function generatePostFile(post) {
    const { categories, tags } = inferCategoriesAndTags(
        post.slug,
        post.title,
        post.content
    )
