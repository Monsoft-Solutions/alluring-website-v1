...
async function processImages() {
  const results = [];
  const imagePosts = postsData.filter(p => p.featuredImage && p.featuredImage.url);
  
  console.log(`Processing ${imagePosts.length} images out of ${postsData.length} posts...\n`);
  
  for (let i = 0; i < imagePosts.length; i++) {
    const post = imagePosts[i];
    
    console.log(`[${i + 1}/${imagePosts.length}] Processing image for ${post.slug}...`);
    ...
    try {
      ...
      console.log(`  ✓ Downloaded (${(metadata.fileSize / 1024).toFixed(1)} KB)`);
  ...
  fs.writeFileSync('/tmp/all-image-processing-results.json', JSON.stringify(results, null, 2));
