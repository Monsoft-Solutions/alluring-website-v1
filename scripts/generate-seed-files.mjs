...
const postsData = JSON.parse(
    fs.readFileSync('/tmp/scraped-posts.json', 'utf-8')
)
const imageResults = JSON.parse(
    fs.readFileSync('/tmp/image-processing-results.json', 'utf-8')
)
...
console.log(
    `✓ Generated ${imageResults.filter((r) => r.localPath).length} image files`
)
